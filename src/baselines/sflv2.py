...
#
# SFLV2 = Split Learning + FedAvg on client-side only
# Server-side model is a single shared global model (like plain SL).
# Client-side models are averaged after each round (like FL).
# ============================================================================
import argparse
import torch
...
program = "SFLV2 ResNet18 on CIFAR10"
print(f"---------{program}----------")
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
    global net_glob_server, criterion, device, batch_acc_train, batch_loss_train
    ...
    # SFLV2: single shared server model (no per-client copy)
...
# ============================================================================
#  Training loop — SFLV2: FedAvg on client only, shared server model
# ============================================================================
...
for iter in range(args.epochs):
    ...
    # Client-side federation only (server model stays shared/global)
    print("------------------------------------------------------------")
...
if args.save_client_ckpt:
    ckpt_path = save_client_checkpoint(net_glob_client, "sflv2", args.cut_layer,
